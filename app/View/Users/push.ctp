<head>  
	...
	<style type="text/css">  
		...
		.test{
			background:-webkit-gradient(linear,0 0,0 100%, color-stop(0.0,rgba(11,22,30,0.8)), color-stop(1.0,rgba(10,37,51,0.5)));
			box-shadow: 0px 0px 70px rgba(0,255,255,0.3);
			z-index: 100;
			position: absolute;
			left: 0px;
			top: 0px;
...
<body>
	<div id="one" class="pictBlock">
		<img class="image" src="<?php echo $this->Html->url(array('controller' => 'Files', 'action' => 'getImg'), array($pictureId))?>">
	</div>
	<nav class="top-bar test" data-topbar role="navigation" data-options="sticky_on: large"></nav>

	<div id="firstModal" class="reveal-modal large" data-reveal  style="text-align: center;">
	  <img src="<?php echo $this->Html->url(array('controller' => 'Files', 'action' => 'getImg'), array($pictureId))?>">
